...
use wasm_context::{Context, Export, FnTy, FunctionKind, Import, Instruction};

pub fn process(ctx: &mut Context) {
    let already_done = ctx.functions.iter().any(|(_, function)| match function {
        &FunctionKind::Import {
            import: Import {
                ref module,
                ref field,
            ...
            ..
        } if module == "env" && field == "__web_on_grow" => true,
        _ => false,
    });
