...
use wasm_context::{
    Context, Export, FnTy, FunctionIndex, FunctionKind, Import, Instruction, TypeIndex,
};
...
pub fn process_and_extract(ctx: &mut Context) -> Vec<JsSnippet> {
    ...
    for (_, function) in &ctx.functions {
        if let &FunctionKind::Definition {
            ref instructions, ..
        } = function
        {
    ...
    fn add_js_snippet(
        ctx: &mut Context,
        value_slice: &[u8],
        snippet_index_by_hash: &mut HashMap<String, usize>,
        snippet_index_by_offset: &mut HashMap<i32, usize>,
    ...
    for (offset, type_index) in snippet_offset_to_type_index {
        ...
        let (data, data_offset) = data_entries
            .iter()
            .filter_map(|data| {
            ...
            .find(|&(data, data_offset)| {
                offset >= data_offset && offset < (data_offset + data.value.len() as i32)
            })
        ...
        add_js_snippet(
            ctx,
            slice,
            &mut snippet_index_by_hash,
            &mut snippet_index_by_offset,
            &mut snippets,
            offset,
            type_index,
        );
    ...
    ctx.patch_code(|instructions| {
        let should_process = instructions.iter().any(|instruction| match instruction {
    ...
    output.extend(snippets.into_iter().map(|snippet| JsSnippet {
        name: snippet.name,
        code: snippet.code,
