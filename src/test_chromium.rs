...
use build::Backend;
use chrome_devtools::{Connection, ConsoleApiCalledBody, ExceptionThrownBody, Reply, ReplyError};
use cmd_test::TEST_RUNNER;
use error::Error;
use http_utils::{response_from_data, response_from_status, SimpleServer};
use utils::{find_cmd, read, read_bytes};
...
pub fn test_in_chromium(
    ...
    let arg_passthrough: Vec<_> = arg_passthrough
        .iter()
        .map(|arg| arg.to_str().unwrap())
        .collect();
    template_data.insert("arguments", arg_passthrough.into());
    template_data.insert("target", backend.triplet().into());
    let test_index = handlebars
        .render_template(DEFAULT_TEST_INDEX_HTML, &template_data)
        .unwrap();
    ...
    thread::spawn(move || {
        let server = SimpleServer::new(&"127.0.0.1:0".parse().unwrap(), move |request| {
            ...
            if path == "/" || path == "index.html" {
                response_from_data("text/html", test_index.clone().into_bytes(), false)
            } else if path == "/js/app.js" {
                let data = server_app_js.lock().unwrap().clone();
                response_from_data("application/javascript", data.into_bytes(), false)
                ...
                match *server_wasm_url.lock().unwrap() {
                    Some(ref server_wasm_url) if path == *server_wasm_url => {
                        ...
                        response_from_data("application/wasm", data, false)
                    }
    ...
    if backend.is_any_wasm() {
        ...
        *wasm_url.lock().unwrap() = Some(format!(
            "/{}",
            wasm_artifact.file_name().unwrap().to_str().unwrap()
        ));
    ...
    *app_js.lock().unwrap() = read(artifact).unwrap();
    ...
    let tmpdir = tempfile::Builder::new()
        .prefix("cargo-web-chromium-profile")
        .tempdir()
        .unwrap();
    ...
    let url = url_rx
        ...
        .map_err(|err| {
            Error::RuntimeError(
                "timeout while waiting for chromium to start".into(),
                err.into(),
            )
        })?;

    debug!("Chromium in listening on: {}", url);
    let mut connection = Connection::connect(&format!("{}/json", url)).map_err(|err| {
        Error::RuntimeError("devtools connection to chromium failed".into(), err.into())
    })?;
    ...
    connection.send_cmd(
        "Page.navigate",
        json!({ "url": format!("http://localhost:{}", server_address.port()) }),
    );
    ...
    loop {
        ...
        let reply = match reply {
            ...
            Err(err) => {
                return Err(Error::RuntimeError(
                    "error while communicating with chromium".into(),
                    err.into(),
                ));
            }
        };
        ...
        match reply {
            ...
            Reply::Event {
                ...
                let body: ExceptionThrownBody = serde_json::from_value(body.clone())
                    .expect("Failed to parse `Runtime.exceptionThrown` event");
                ...
                if let Some(url) = body.exception_details.url {
                    eprintln!(
                        "error: source: {}:{}:{}",
                        url,
                        body.exception_details.line_number,
                        body.exception_details.column_number
                    );
                }
            ...
            Reply::Error { ref message, .. } => {
                return Err(Error::RuntimeError(
                    "chromium returned an error".into(),
                    message.clone().into(),
                ));
            }
            _ => {}
