...
use pbr;
use reqwest::{header, Client, Proxy, Result as ReqResult, Url};
use sha2;
...
fn create_client() -> ReqResult<Client> {
    ...
    match env::var("HTTPS_PROXY") {
        Err(_) => {}
        Ok(proxy) => {
            builder = builder.proxy(Proxy::https(&proxy).unwrap());
        }
    };
...
pub fn download_package(package: &PrebuiltPackage) -> PathBuf {
    ...
    if actual_hash != package.hash {
        eprintln!(
            "error: the hash of {} doesn't match the expected hash!",
            package_filename
        );
