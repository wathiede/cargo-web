...
impl TargetCfg {
    ...
    fn matches_value(&self, value: &Cfg) -> bool {
        match *value {
            Cfg::Name(ref name) => self.set.contains(name),
            Cfg::KeyPair(ref key, ref expected_value) => self
                .map
                .get(key)
...
impl fmt::Display for Error {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            Error::CannotLaunchCargo(ref err) => {
                write!(formatter, "{}: {}", self.description(), err)
            }
            Error::CargoFailed(ref err) => write!(formatter, "{}: {}", self.description(), err),
            Error::CannotParseCargoOutput(ref err) => {
                write!(formatter, "{}: {}", self.description(), err)
            }
        }
    }
...
impl CargoProject {
    pub fn new(
        ...
        let cargo = env::var("CARGO")
            .unwrap_or_else(|_| if cfg!(windows) { "cargo.exe" } else { "cargo" }.to_owned());
        ...
        let output = command
            .output()
            .map_err(|err| Error::CannotLaunchCargo(err))?;
                        ...
                        targets: package
                            ...
                            .filter_map(|target| {
                                Some(CargoTarget {
                                    ...
                                    source_directory: Into::<PathBuf>::into(target.src_path)
                                        .parent()
                                        .unwrap()
                                        .into(),
                                })
                            ...
                            .map(|dependency| {
                                ...
                                let target = match json.get("target").unwrap() {
                                    ...
                                    &serde_json::Value::String(ref target) => {
                                        let target = if target.starts_with("cfg(")
                                            && target.ends_with(")")
                                        {
                                            let cfg = target[4..target.len() - 1]
                                                .trim()
                                                .parse()
                                                .expect(
                                                "cannot parse target specification in a Cargo.toml",
                                            );
        ...
        for node in metadata
            .resolve
            .expect("missing `resolve` metadata section")
            .nodes
        {
            let id = CargoPackageId::new(&node.id)
                .expect("unparsable package id in the `resolve` metadata section");
            let package_index = *package_map
                .get(&id)
                .expect("extra entry in the `resolve` metadata section");
            let package = &mut project.packages[package_index];
            for dependency_id in node.dependencies {
                let dependency_id =
                    CargoPackageId::new(&dependency_id).expect("unparsable dependency package id");

                let mut dependency_found = false;
                for dependency in package
                    .dependencies
                    .iter_mut()
                    .filter(|dep| dep.name == dependency_id.name())
                {
        ...
        for (package_index, package) in project.packages.iter().enumerate() {
            ...
            let common_length = cwd
                .components()
                .zip(package_directory.components())
                .take_while(|&(a, b)| a == b)
                .count();
    ...
    pub fn used_packages(
        ...
        while let Some(index) = queue.pop() {
            for dependency in &entries[index].package.dependencies {
                ...
                match profile {
                    Profile::Main => match dependency.kind {
                        CargoDependencyKind::Normal => {}
                        CargoDependencyKind::Development | CargoDependencyKind::Build => continue,
                    },
                    Profile::Test | Profile::Bench => match dependency.kind {
                        CargoDependencyKind::Normal | CargoDependencyKind::Development => {}
        ...
        entries
            .into_iter()
            .filter(|entry| entry.is_used.get())
...
impl BuildConfig {
    ...
    where
        F: for<'a> FnMut(Vec<PathBuf>) -> Vec<PathBuf>,
    {
    ...
    fn launch_cargo(&self, should_build: bool) -> Option<(i32, Vec<cargo_output::Artifact>)> {
        ...
        for &(ref key, ref value) in &self.extra_environment {
            debug!(
                "Will launch cargo with variable \"{}\" set to \"{}\"",
                key, value
            );
        ...
        for line in stdout.lines() {
            ...
            let json: serde_json::Value =
                serde_json::from_str(&line).expect("failed to parse cargo output");
            let line = serde_json::to_string_pretty(&json).unwrap();
            if let Some(output) = CargoOutput::parse(&line) {
                match output {
                    CargoOutput::Message(message) => match self.message_format {
                        MessageFormat::Human => {
                            diagnostic_formatter::print(self.use_color, &message)
                        }
                        MessageFormat::Json => {
                            println!(
                                "{}",
                                serde_json::to_string(&message.to_json_value()).unwrap()
                            );
                        }
                    ...
                    CargoOutput::BuildScriptExecuted(executed) => match self.message_format {
                        MessageFormat::Human => {}
                        MessageFormat::Json => {
                            println!(
                                "{}",
                                serde_json::to_string(&executed.to_json_value()).unwrap()
                            );
                        }
                        MessageFormat::__Nonexhaustive => unreachable!(),
                    },
        ...
        let status = result
            .unwrap()
            .code()
            .expect("failed to grab cargo status code");
        ...
        F: for<'a> FnMut(Vec<PathBuf>) -> Vec<PathBuf>,
    {
        ...
        fn find_artifact(
            artifacts: &[cargo_output::Artifact],
            extension: &str,
        ) -> Option<(usize, usize)> {
            artifacts
                .iter()
                ...
                .filter_map(|(artifact_index, artifact)| {
                    if let Some(filename_index) = artifact
                        .filenames
                        .iter()
                        .position(|filename| has_extension(filename, extension))
                    {
        ...
        if status == 0
            && self
                .triplet
                .as_ref()
                .map(|triplet| triplet == "wasm32-unknown-emscripten")
                .unwrap_or(false)
        {
            match self.build_target {
                ...
                | BuildTarget::IntegrationTest(_) => {
                    if find_artifact(&artifacts, "wasm").is_none() {
                        if let Some((artifact_index, filename_index)) =
                            find_artifact(&artifacts, "js")
                        {
                            let wasm_path = {
                                let main_artifact =
                                    Path::new(&artifacts[artifact_index].filenames[filename_index]);
                            ...
                            assert!(
                                wasm_path.exists(),
                                "internal error: wasm doesn't exist where I expected it to be"
                            );
                            artifacts[artifact_index]
                                .filenames
                                .push(wasm_path.to_str().unwrap().to_owned());
        ...
        for mut artifact in &mut artifacts {
            if let Some(ref mut callback) = postprocess.as_mut() {
                let filenames = artifact
                    .filenames
                    .iter()
                    .map(|filename| Path::new(&filename).to_owned())
                    .collect();
        ...
        for mut artifact in artifacts {
            ...
            match self.message_format {
                ...
                MessageFormat::Json => {
                    println!(
                        "{}",
                        serde_json::to_string(&artifact.to_json_value()).unwrap()
                    );
                }
                MessageFormat::__Nonexhaustive => unreachable!(),
            }
