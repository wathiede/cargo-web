...
impl error::Error for Error {
    fn description(&self) -> &str {
        match *self {
            ...
            Error::NoDefaultPackage => {
                "no default package; you can specify a crate to use with the `-p` argument"
            }
            Error::EmscriptenNotAvailable => {
                "prepackaged Emscripten is not available for this platform"
            }
...
impl fmt::Display for Error {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            &Error::RuntimeError(_, ref inner) => {
                write!(formatter, "{}: {}", self.description(), inner)
            }
            &Error::CargoShimError(cargo_shim::Error::CargoFailed(ref message)) => {
                write!(formatter, "{}", message)
            }
            &Error::CargoShimError(ref inner) => write!(formatter, "{}", inner),
            &Error::CannotLoadFile(ref path, ref inner) => {
                write!(formatter, "cannot load file {:?}: {}", path, inner)
            }
            &Error::CannotRemoveDirectory(ref path, ref inner) => {
                write!(formatter, "cannot remove directory {:?}: {}", path, inner)
            }
            &Error::CannotRemoveFile(ref path, ref inner) => {
                write!(formatter, "cannot remove file {:?}: {}", path, inner)
            }
            &Error::CannotCreateFile(ref path, ref inner) => {
                write!(formatter, "cannot create file {:?}: {}", path, inner)
            }
            &Error::CannotWriteToFile(ref path, ref inner) => {
                write!(formatter, "cannot write to file {:?}: {}", path, inner)
            }
            &Error::CannotCopyFile(ref src_path, ref dst_path, ref inner) => write!(
                formatter,
                "cannot copy file from {:?} to {:?}: {}",
                src_path, dst_path, inner
            ),
