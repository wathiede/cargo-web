...
use cargo_shim::{
    target_to_build_target, BuildConfig, BuildType, CargoPackage, CargoProject, CargoResult,
    CargoTarget, MessageFormat, Profile, TargetKind,
};
use semver::Version;
use serde_json;
use walkdir::WalkDir;
...
fn get_package<'a>(name: Option<&str>, project: &'a CargoProject) -> Result<usize, Error> {
    if let Some(name) = name {
        ...
        project
            .packages
            .iter()
            .position(|package| package.is_default)
            .ok_or(Error::NoDefaultPackage)
    }
}

fn get_target<'a>(
    kind: &Option<TargetName>,
    ...
    match *kind {
        TargetName::Lib => {
            match targets
                .iter()
                .position(|target| target.kind == TargetKind::Lib)
            {
                None => {
                    return Err(Error::ConfigurationError(format!(
        ...
        TargetName::Bin(ref name) => {
            match targets
                .iter()
                .position(|target| target.kind == TargetKind::Bin && target.name == *name)
            {
                None => {
                    return Err(Error::ConfigurationError(format!(
                        "no bin target named `{}`",
        ...
        TargetName::Example(ref name) => {
            match targets
                .iter()
                .position(|target| target.kind == TargetKind::Example && target.name == *name)
            {
                None => {
                    return Err(Error::ConfigurationError(format!(
                        "no example target named `{}`",
                        name
                    )));
                }
                index => Ok(index),
        ...
        TargetName::Bench(ref name) => {
            match targets
                .iter()
                .position(|target| target.kind == TargetKind::Bench && target.name == *name)
            {
                None => {
                    return Err(Error::ConfigurationError(format!(
                        "no bench target named `{}`",
                        name
                    )));
                }
                index => Ok(index),
...
impl Project {
    pub fn new(args: BuildArgs) -> Result<Self, Error> {
        ...
        let default_package = get_package(
            args.package_name.as_ref().map(|name| name.as_str()),
            &project,
        )?;
        let default_target = get_target(&args.target_name, &project.packages[default_package])?;

        let main_config =
            Config::load_for_package_printing_warnings(&project.packages[default_package], true)?;
        ...
        if project.build_args.runtime != RuntimeKind::Standalone
            && !project.backend().is_native_wasm()
        {
            return Err(format!(
                "`--runtime` can be only used with `--target=wasm32-unknown-unknown`"
            )
            .into());
        }
    ...
    pub fn backend(&self) -> Backend {
        self.build_args
            ...
            .or_else(|| {
                self.main_config
                    .as_ref()
                    .and_then(|config| config.default_target)
            })
    ...
    where
        for<'r> F: Fn(&'r CargoTarget) -> bool,
    {
        let package = self.package();
        Ok(self
            .default_target
            .map(|target| vec![&package.targets[target]])
            .unwrap_or_else(|| {
                package
                    .targets
                    .iter()
    ...
    fn used_packages(&self, profile: Profile) -> Vec<&CargoPackage> {
        ...
        let mut packages =
            self.project
                .used_packages(self.backend().triplet(), main_package, profile);

        packages.sort_by(|a, b| {
            (!(*a == main_package), !a.is_workspace_member, &a.name).cmp(&(
                !(*b == main_package),
    ...
    pub fn aggregate_configuration(&self, profile: Profile) -> Result<AggregatedConfig, Error> {
            ...
            if let Some(ref config) = config {
                if let Some(ref new_requirement) = config.minimum_cargo_web_version {
                    ...
                    match maximum_minimum_version.take() {
                        Some((_, ref previous_requirement))
                            if *new_requirement > *previous_requirement =>
                        {
                            maximum_minimum_version =
                                Some((config.source(), new_requirement.clone()));
                        }
        ...
        if let Some((ref requirement_source, ref minimum_version)) = maximum_minimum_version {
            if current_version < *minimum_version {
                return Err(format!(
                    "{} requires at least `cargo-web` {}; please update",
                    requirement_source, minimum_version
                )
                .into());
            }
        }
        ...
        for config in configs.iter().rev() {
            if let Some(ref config) = *config {
                if let Some(ref link_args) = config.get_link_args(self.backend()) {
                    debug!(
                        "{} defines the following link-args: {:?}",
                        config.source(),
                        link_args
                    );
                    aggregated_config
                        .link_args
                        .extend(link_args.iter().cloned());
                }

                if let Some(ref prepend_js) = config.get_prepend_js(self.backend()) {
                    debug!(
                        "{} wants to prepend the following JS files: {:?}",
                        config.source(),
                        prepend_js
                    );
                    ...
                    for path in prepend_js.iter() {
                        ...
                        if !full_path.exists() {
                            return Err(format!(
                                "{}: file specified by 'prepare-js' not found: {:?}",
                                config.source(),
                                path
                            )
                            .into());
                        ...
                        let contents = read(&full_path).map_err(|err| {
                            format!("{}: cannot read {:?}: {}", config.source(), path, err)
                        })?;
    ...
    fn prepare_build_config(&self, config: &AggregatedConfig, target: &CargoTarget) -> BuildConfig {
        ...
        let vanilla_emscripten_build = env::var("CARGO_WEB_VANILLA_EMSCRIPTEN_BUILD")
            .map(|value| value == "1")
            .unwrap_or(false)
        ...
        if self.backend().is_emscripten() {
            if let Some(emscripten) = initialize_emscripten(
                self.build_args.use_system_emscripten,
                self.backend().is_emscripten_wasm(),
            ) {
                ...
                let emscripten_llvm_path = emscripten
                ...
                extra_environment.push((
                    "EMSCRIPTEN_FASTCOMP".to_owned(),
                    emscripten_llvm_path.clone(),
                ));
            ...
            extra_rustflags.push(format!(
                "link-arg=NO_EXIT_RUNTIME={}",
                no_exit_runtime as u32
            ));
            ...
            if !vanilla_emscripten_build {
                ...
                extra_rustflags.push("-C".to_owned());
                extra_rustflags.push(format!(
                    "link-arg=ALLOW_MEMORY_GROWTH={}",
                    allow_memory_growth as u32
                ));
        ...
        if self.backend().is_native_wasm()
            && self.build_args.build_type == BuildType::Debug
            && config.uses_old_stdweb
        {
            extra_rustflags.push("-C".to_owned());
        ...
        let build_type = if self.backend().is_native_wasm()
        ...
        if !extra_emmaken_cflags.is_empty() && !vanilla_emscripten_build {
            ...
            let emmaken_cflags: Vec<_> = extra_emmaken_cflags
                .into_iter()
        ...
        extra_environment.push((
            "CARGO_WEB_TARGET_DIR".to_owned(),
            self.target_directory().to_str().unwrap().to_owned(),
        ));
        extra_environment.push((
            "CARGO_WEB_TARGET".to_owned(),
            self.backend().triplet().to_owned(),
        ));
    ...
    pub fn static_paths(package: &CargoPackage, target: &CargoTarget) -> Vec<PathBuf> {
        ...
        let target_static_path = match target.kind {
            TargetKind::Example => Some(
                target
                    .source_directory
                    .join(format!("{}-static", target.name)),
            ),
    ...
    pub fn paths_to_watch(
        &self,
        target: &CargoTarget,
    ) -> Vec<(PathBuf, PathKind, ShouldTriggerRebuild)> {
        // TODO: `Web.toml` and `prepend-js` support.
        let mut paths = Vec::new();
        paths.push((
            target.source_directory.clone(),
            PathKind::Directory,
            ShouldTriggerRebuild::Yes,
        ));
        ...
        for package in packages {
            ...
            if let Some(lib_target) = package
                .targets
                .iter()
                .find(|target| target.kind == TargetKind::Lib || target.kind == TargetKind::CDyLib)
            {
                paths.push((
                    lib_target.source_directory.clone(),
                    PathKind::Directory,
                    ShouldTriggerRebuild::Yes,
                ));
            }
    ...
    fn install_target_if_necessary(&self) -> Result<(), Error> {
        ...
        let output = Command::new(rustup)
            ...
            .map_err(|err| {
                Error::RuntimeError(
                    "cannot get the target list through rustup".into(),
                    err.into(),
                )
            })?;
        ...
        if !output.status.success() {
            return Err(
                "cannot get the target list through rustup: rustup invocation failed".into(),
            );
        }
        ...
        match targets.get(self.backend().triplet()).cloned() {
            Some(false) => {
                ...
                let result = result.map_err(|err| {
                    Error::RuntimeError(
                        format!(
                            "installation of target `{}` through rustup failed",
        ...
        let result = if !should_build {
            ...
            build_config.build(Some(|artifacts: Vec<PathBuf>| {
                ...
                for path in artifacts {
                    if let Some(artifact) = wasm::process_wasm_file(
                        config.uses_old_stdweb,
                        self.build_args.runtime,
                        &build_config,
                        &prepend_js,
                        target_dir,
                        &path,
