...
fn from_string_or_array_of_strings(
    path_in_toml: &str,
    config: &Config,
    prepend_js: toml::Value,
) -> Result<Vec<String>, Error> {
    match prepend_js {
        toml::Value::String(path) => Ok(vec![path]),
        toml::Value::Array(ref array) if is_all_strings(&array) => Ok(array
            .iter()
...
impl Config {
    ...
    pub fn get_link_args(&self, backend: Backend) -> Option<&Vec<String>> {
        self.per_target
            .get(&backend)
            .and_then(|per_target| per_target.link_args.as_ref())
    }

    pub fn get_prepend_js(&self, backend: Backend) -> Option<&Vec<String>> {
        self.per_target
            .get(&backend)
            .and_then(|per_target| per_target.prepend_js.as_ref())
    }
}
...
fn add_link_args(
    ...
    return Err(format!(
        "{}: you can't have multiple 'link-args' defined for a single target",
        config.source()
    )
    .into());
}
...
fn add_prepend_js(
    ...
    return Err(format!(
        "{}: you can't have multiple 'prepend-js' defined for a single target",
        config.source()
    )
    .into());
}
...
impl Config {
    pub fn load_from_file<P>(
        ...
        crate_name: Option<String>,
        is_main_crate: bool,
    ) -> Result<Option<(Self, Vec<Warning>)>, Error>
    where
        P: AsRef<Path>,
    {
            ...
            toml::Value::Table(table) => {
                for (toplevel_key, toplevel_value) in table {
                    match toplevel_key.as_str() {
                        ...
                        "link-args" => {
                            let link_args: Vec<String> =
                                toplevel_value.try_into().map_err(|_| {
                                    format!(
                                        "{}: 'link-args' is not an array of strings",
                                        config.source()
                                    )
                                })?;
                            ...
                            warnings.push(Warning::Deprecation(
                                "link-args".to_owned(),
                                Some(
                                    "it should be moved to the '[target.emscripten]' section"
                                        .to_owned(),
                                ),
                            ));
                        ...
                        "prepend-js" => {
                            let toplevel_value = from_string_or_array_of_strings(
                                &toplevel_key,
                                &config,
                                toplevel_value,
                            )?;
                        ...
                        "default-target" => {
                            ...
                            let default_target = match default_target.as_str() {
                                ...
                                _ => {
                                    if is_main_crate {
                                        return Err(format!(
                                            "{}: `default-target` has an invalid value: `{}`",
                                            config.source(),
                                            default_target
                                        )
                                        .into());
                        ...
                        "cargo-web" => {
                            let cargo_web_table: toml::value::Table =
                                toplevel_value.try_into().map_err(|_| {
                                    format!("{}: 'cargo-web' should be a section", config.source())
                                })?;
                            ...
                            for (cargo_web_key, cargo_web_value) in cargo_web_table {
                                match cargo_web_key.as_str() {
                                    ...
                                    cargo_web_key => {
                                        warnings.push(Warning::UnknownKey(format!(
                                            "cargo-web.{}",
                                            cargo_web_key
                                        )));
                                    }
                        ...
                        "target" => {
                            let target_table: toml::value::Table =
                                toplevel_value.try_into().map_err(|_| {
                                    format!("{}: 'target' should be a section", config.source())
                                })?;
                            ...
                            for (target_key, target_value) in target_table {
                                let backends = match target_key.as_str() {
                                    ...
                                    "asmjs-unknown-emscripten" => &[Backend::EmscriptenAsmJs][..],
                                    "emscripten" => {
                                        &[Backend::EmscriptenWebAssembly, Backend::EmscriptenAsmJs]
                                            [..]
                                    }
                                    target_key => {
                                        warnings.push(Warning::UnknownKey(format!(
                                ...
                                let target_subtable: toml::value::Table =
                                    target_value.try_into().map_err(|_| {
                                        format!(
                                            "{}: 'target.{}' should be a section",
                                            config.source(),
                                            target_key
                                        )
                                    })?;
                                ...
                                for (per_target_key, per_target_value) in target_subtable {
                                    let path_in_toml =
                                        format!("target.{}.{}", target_key, per_target_key);
                                    match per_target_key.as_str() {
                                        ...
                                        "prepend-js" => {
                                            let per_target_value = from_string_or_array_of_strings(
                                                &path_in_toml,
                                                &config,
                                                per_target_value,
                                            )?;
    ...
    pub fn load_for_package_printing_warnings(
        package: &CargoPackage,
        is_main_crate: bool,
        ...
        for warning in warnings {
            match warning {
                ...
                Warning::InvalidValue(key) => {
                    eprintln!(
                        "warning: key `{}` in {} has an invalid value",
                        key,
                        config.source()
                    );
                }
