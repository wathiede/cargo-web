...
use cargo_shim::{CargoPackage, CargoResult, CargoTarget, TargetKind};
...
fn generate_index_html(filename: &str) -> String {
    ...
    handlebars
        .render_template(DEFAULT_INDEX_HTML_TEMPLATE, &template_data)
        .unwrap()
}
...
impl Deployment {
    ...
    pub fn js_url(&self) -> &str {
        let route = self
            .routes
            .iter()
            .find(|route| route.can_be_deployed && route.key.ends_with(".js"))
            .unwrap();
        &route.key
    }
    ...
    pub fn get_by_url(&self, mut url: &str) -> Option<Artifact> {
        ...
        let mime_type = if url.ends_with(".js") {
        ...
        for route in &self.routes {
            ...
            match route.kind {
                RouteKind::Blob(ref bytes) => {
                    ...
                    let mut fp = File::create(&target_path)
                        .map_err(|err| Error::CannotCreateFile(target_path.to_owned(), err))?;
                    fp.write_all(&bytes)
                        .map_err(|err| Error::CannotWriteToFile(target_path.to_owned(), err))?;
                }
                    ...
                    for entry in WalkDir::new(source_dir) {
                        ...
                        if source_path.is_dir() {
                            fs::create_dir_all(&target_path).map_err(|err| {
                                Error::CannotCreateFile(target_path.to_owned(), err)
