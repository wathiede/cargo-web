...
trait IterExt: Iterator + Sized {
    fn enumerate_u32(
        self,
    ) -> iter::Map<iter::Enumerate<Self>, fn((usize, Self::Item)) -> (u32, Self::Item)> {
...
impl Export {
    pub fn none() -> Self {
        Export { names: Vec::new() }
    }

    pub fn some(name: String) -> Self {
        Export { names: vec![name] }
    }
}
...
fn decode_name_map<F: for<'a> FnMut(u32, &'a str)>(
    p: &mut &[u8],
    mut callback: F,
) -> Result<(), pw::Error> {
...
fn serialize_name_section(
    ...
    write_with_length(&mut output, move |body| {
        ...
        if !local_names.is_empty() {
            ...
            write_with_length(body, |inner_body| {
                pw::Serialize::serialize(pw::VarUint32::from(local_names.len()), inner_body)
                    .unwrap();
                for (function_index, names) in local_names {
                    pw::Serialize::serialize(pw::VarUint32::from(function_index), inner_body)
                        .unwrap();
...
fn preprocess_entities<T: ImportExport>(map: IndexMap<u32, T>) -> Entities<T> {
    ...
    let (imports, definitions): (Vec<_>, Vec<_>) = map
        .into_iter()
        .partition(|&(_, ref entity)| entity.is_imported());
...
impl Context {
            ...
            match section {
                ...
                pw::Section::Element(mut section) => {
                    ...
                    assert_eq!(
                        entries.len(),
                        1,
                        "multiple Element tables are not supported"
                    );
                ...
                pw::Section::Data(mut section) => {
                    for mut entry in take(section.entries_mut()) {
                        if ctx
                            ...
                            .last()
                            .map(|last_data| {
                                last_data.offset == entry.offset().as_ref().unwrap().code()
                                    && last_data.value.is_empty()
                            })
                            .unwrap_or(false)
                        {
                ...
                pw::Section::Custom(mut section) => {
                    if section.name() == "name" {
                        ...
                        while p.len() > 0 {
                            ...
                            let payload_length =
                                u32::from(pw::VarUint32::deserialize(&mut p).unwrap());
                            ...
                            match kind {
                                ...
                                2 => {
                                    let count = u32::from(
                                        pw::VarUint32::deserialize(&mut payload).unwrap(),
                                    );
                                    for _ in 0..count {
                                        let function_index = u32::from(
                                            pw::VarUint32::deserialize(&mut payload).unwrap(),
                                        );
                                        decode_name_map(
                                            &mut payload,
                                            |local_index, name| match ctx
                                                .functions
                                                .get_mut(&function_index)
                                                .unwrap()
                                            {
                        ...
                        let url = str::from_utf8(&p[0..url_length as usize])
                            .expect("invalid sourceMappingURL");
                ...
                pw::Section::Reloc(_) | pw::Section::Unparsed { .. } => unimplemented!(),
            }
        }
    ...
    pub fn into_module(self) -> pw::Module {
        ...
        for (new_index, function) in functions.entries {
            let export = match function {
                ...
                FunctionKind::Definition {
                    ...
                    section_code.push(pw::FuncBody::new(
                        locals,
                        pw::Instructions::new(instructions),
                    ));
            ...
            for name in export.names {
                section_exports.push(pw::ExportEntry::new(
                    name,
                    pw::Internal::Function(new_index),
                ));
            }
        }
            ...
            let export = match global {
                GlobalKind::Import {
                    ...
                    section_imports.push(pw::ImportEntry::new(
                        import.module,
                        import.field,
                        pw::External::Global(pw::GlobalType::new(
                            global_type.ty,
        ...
        for mut pointer_table in self.fn_pointer_tables {
            process_instructions(
                ...
                &type_map,
                &globals.index_map,
                &mut pointer_table.offset,
            );
            for function_index in &mut pointer_table.members {
                *function_index = functions.index_map.get(&function_index).cloned().unwrap();
            }

            let entry = pw::ElementSegment::new(
                0,
                Some(pw::InitExpr::new(pointer_table.offset)),
                pointer_table.members,
                false,
            );
        ...
        for data in self.data {
            section_data.push(pw::DataSegment::new(
                0,
                Some(pw::InitExpr::new(data.offset)),
                data.value,
                false,
            ));
        }
        ...
        if !section_types.is_empty() {
            sections.push(pw::Section::Type(pw::TypeSection::with_types(
                section_types,
            )));
        }

        if !section_imports.is_empty() {
            sections.push(pw::Section::Import(pw::ImportSection::with_entries(
                section_imports,
            )));
        }

        if !section_functions.is_empty() {
            sections.push(pw::Section::Function(pw::FunctionSection::with_entries(
                section_functions,
            )));
        }

        if !section_tables.is_empty() {
            sections.push(pw::Section::Table(pw::TableSection::with_entries(
                section_tables,
            )));
        }

        if !section_memories.is_empty() {
            sections.push(pw::Section::Memory(pw::MemorySection::with_entries(
                section_memories,
            )));
        }

        if !section_globals.is_empty() {
            sections.push(pw::Section::Global(pw::GlobalSection::with_entries(
                section_globals,
            )));
        }

        if !section_exports.is_empty() {
            sections.push(pw::Section::Export(pw::ExportSection::with_entries(
                section_exports,
            )));
        }
        ...
        if !section_elements.is_empty() {
            sections.push(pw::Section::Element(pw::ElementSection::with_entries(
                section_elements,
            )));
        }

        if !section_code.is_empty() {
            sections.push(pw::Section::Code(pw::CodeSection::with_bodies(
                section_code,
            )));
        }

        if !section_data.is_empty() {
            sections.push(pw::Section::Data(pw::DataSection::with_entries(
                section_data,
            )));
        }
        ...
        if self.module_name.is_some()
            ...
            || !function_variable_names.is_empty()
        {
            let name_section_bytes =
                serialize_name_section(self.module_name, function_names, function_variable_names);
    ...
    pub fn patch_code<F>(&mut self, mut callback: F)
    where
        F: for<'r> FnMut(&'r mut Vec<Instruction>),
    {
...
fn test_serialization_deserialization() {
    ...
    let type_index = ctx.get_or_add_fn_type(FnTy {
...
fn test_function_import_removal() {
    ...
    ctx.add_function(FunctionKind::Definition {
        ...
        instructions: vec![Instruction::Call(1)],
    });
    ctx.start = Some(1);
