    ...
    let target = {
        ...
        if targets.is_empty() {
            return Err(Error::ConfigurationError(format!(
                "cannot start a webserver for a crate which is a library!"
            )));
        }
...
fn should_rebuild(
    paths_to_watch: &[(PathBuf, PathKind, ShouldTriggerRebuild)],
    path: &Path,
) -> bool {
    paths_to_watch
        .iter()
            ...
            if let Ok(project) = build_args.load_project() {
                if let Ok(target) = select_target(&project) {
                    ...
                    if new_paths_to_watch != last_paths_to_watch {
                        debug!(
                            "Paths to watch have changed; new paths to watch: {:#?}",
                            new_paths_to_watch
                        );
...
pub fn command_start(
        ...
        if let Some(mut artifact) = last_build.deployment.get_by_url(&path) {
            if auto_reload && (path == "/" || path == "/index.html") {
                let result = artifact.map_text(|text| {
                    ...
                    text.replace(
                        "<head>",
                        &format!("<head><script>{}</script>", injected_code),
                    )
                });
    ...
    match target.kind {
        TargetKind::Example => eprintln!(
            "You can also put a '{}-static' directory in your 'examples' directory.",
            target.name
        ),
        TargetKind::Bin | TargetKind::CDyLib => {
            eprintln!("You can also put a 'static' directory in your 'src' directory.")
        }
        _ => unreachable!(),
    };
    eprintln!("");
    eprintln!(
        "Your application is being served at '/{}'. It will be automatically",
        js_url
    );
