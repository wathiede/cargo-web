...
fn to_js_identifier(string: &str) -> String {
    ...
    string
        .chars()
        .take(1)
        .map(replace_invalid_starting_char)
        .chain(string.chars().skip(1).map(replace_invalid_middle_char))
        .collect()
}
...
pub fn generate_js(
    ...
    for export in exports {
        ...
        let arg_names = join(
            ", ",
            export.metadata.args.iter().map(|arg| arg.name.as_str()),
        );
        writeln!(code, "function {}({}) {{", export.metadata.name, arg_names).unwrap();

        let arg_conversions = join(
            ", ",
            export.metadata.args.iter().map(|arg| match arg.ty {
                TypeMetadata::I32 | TypeMetadata::F64 => format!("{}", arg.name),
                TypeMetadata::Custom {
                    ref conversion_fn, ..
                } => format!("{}({})", conversion_fn, arg.name),
            }),
        );

        let call = format!(
            "Module.instance.exports.{}({})",
            export.raw_name, arg_conversions
        );
    ...
    if let Some(main_symbol) = main_symbol {
        template_data.insert(
            "call_main",
            format!("Module.instance.exports.{}();", main_symbol),
        );
    } else {
        template_data.insert("call_main", "".to_owned());
    }

    let factory = handlebars
        .render_template(FACTORY_TEMPLATE, &template_data)
        .unwrap();
    template_data.clear();
    ...
    match runtime {
        RuntimeKind::Standalone => handlebars
            .render_template(STANDALONE_TEMPLATE, &template_data)
            .unwrap(),
        RuntimeKind::LibraryEs6 => handlebars
            .render_template(LIBRARY_ES6_TEMPLATE, &template_data)
            .unwrap(),
        RuntimeKind::WebExtension => handlebars
            .render_template(WEB_EXTENSION_TEMPLATE, &template_data)
            .unwrap(),
        RuntimeKind::OnlyLoader => {
            // TODO: Get rid of this.
            handlebars
                .render_template(ONLY_LOADER_TEMPLATE, &template_data)
                .unwrap()
        }
    }
}
