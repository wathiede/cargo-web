...
use cargo_shim::{Profile, TargetKind};
...
pub fn command_deploy(build_args: BuildArgs, directory: Option<PathBuf>) -> Result<(), Error> {
    ...
    if targets.is_empty() {
        if project.backend().is_native_wasm() {
            return Err(
                "No valid target found for deployment; expected a `bin` crate or a `cdylib`".into(),
            );
        } else {
    ...
    if directory.exists() && is_using_default_directory {
        let entries = fs::read_dir(&directory)
            .map_err(|error| Error::CannotRemoveDirectory(directory.clone(), error))?; // TODO: Another error?
        for entry in entries {
            let entry = entry.expect("cannot unwrap directory entry");
            let path = entry.path();
            if path.is_dir() {
                fs::remove_dir_all(&path)
                    .map_err(|error| Error::CannotRemoveDirectory(path.clone(), error))?;
            } else {
                fs::remove_file(&path)
                    .map_err(|error| Error::CannotRemoveFile(path.clone(), error))?;
            }
        }
    }
