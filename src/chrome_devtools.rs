...
impl fmt::Display for ConnectionError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ConnectionError::FailedToFetchUrl(ref message) => {
                write!(fmt, "{}: {}", self.description(), message)
            }
            ConnectionError::WebSocketError(ref message) => {
                write!(fmt, "{}: {}", self.description(), message)
            }
        }
    }
...
impl fmt::Display for CommunicationError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            CommunicationError::Send(ref message) => {
                write!(fmt, "{}: {}", self.description(), message)
            }
            CommunicationError::Recv(ref message) => {
                write!(fmt, "{}: {}", self.description(), message)
            }
        }
    }
...
impl fmt::Display for ReplyError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            ReplyError::MalformedMessage(ref error) => {
                write!(fmt, "{}: {}", self.description(), error)
            }
            ReplyError::MalformedJson(ref error) => {
                write!(fmt, "{}: {}", self.description(), error)
            }
            ReplyError::CommunicationError(ref error) => {
                write!(fmt, "{}: {}", self.description(), error)
            }
        }
    }
...
fn owned_message_to_json(
    message: Result<OwnedMessage, CommunicationError>,
...
impl Connection {
    pub fn connect(json_url: &str) -> Result<Self, ConnectionError> {
        ...
        let json = initial_json
            .expect("timed out while waiting for Chromium to give us websocket debugger URL");
        let url = json
            .get(0)
            .expect("can't find first element")
            .get("webSocketDebuggerUrl")
            .expect("no webSocketDebuggerUrl found")
            .as_str()
            .expect("webSocketDebuggerUrl is not a string");
        ...
        thread::spawn(move || {
            for message in receiver.incoming_messages() {
                ...
                match message {
                    ...
                    message @ OwnedMessage::Ping(_) => match output_tx_clone.send(message) {
                        Ok(()) => {}
                        Err(_) => break,
                    },
                    message => match input_tx.send(Ok(message)) {
                        Ok(()) => {}
                        Err(_) => break,
                    },
                }
    ...
    ) -> Option<Result<OwnedMessage, CommunicationError>> {
    ...
    pub fn try_recv(&mut self, wait_for: Option<Duration>) -> Result<Reply, ReplyError> {
        ...
        if let Some(error) = json.get("error") {
            ...
            let code = error
                ...
                .as_i64()
                .ok_or(ReplyError::MalformedMessage(
                    "'error.code' field is not an integer",
                ))?;
            let message = error
                .get("message")
                .ok_or(ReplyError::MalformedMessage(
                ...
                .as_str()
                .ok_or(ReplyError::MalformedMessage(
                    "'error.message' field is not an integer",
                ))?
                .to_owned();
            ...
            let body = json
                .get("result")
                .ok_or(ReplyError::MalformedMessage("'body' field not found"))?
                .clone();
            ...
            let body = json
                .get("params")
                .ok_or(ReplyError::MalformedMessage("'params' field not found"))?
                .clone();
